// BioSense Band - Firmware v5.7.2 - Pulido
#include <BLEDevice.h>
#include <BLEServer.h>
#include <BLEUtils.h>
#include <BLE2902.h>
...
#define PIN_SDA 6
#define PIN_SCL 7
#define PIN_NIR 34  // ⬅️ CAMBIADO: pin seguro, no conflictivo
#define PIN_LED 8
...
bool initMAX30105() {  // ⬅️ CORREGIDO nombre
...
float readMLX90614() {
  ...
  return temp_c * 0.6 + tempC * 0.4;  // ⬅️ más ágil
}
...
void readMPU6050() {
  ...
  Wire.requestFrom(ADDR_MPU6050, 6);
  ...
  ay = ay_raw / 16384.0f;
  az = az_raw / 16384.0f;
  motion_magnitude = fabs(sqrtf(ax*ax + ay*ay + az*az) - 1.0f);
...
void saveDarwinWeights() {
  prefs.begin("darwin", false);
  prefs.putFloat("w0", darwin_weights[0]);
  prefs.putFloat("w1", darwin_weights[1]);
...
void normalizeWeights() {
  ...
  for (int iter = 0; iter < 10; iter++) {
    float sum = 0;
    for (int i = 0; i < N; i++) sum += darwin_weights[i];
    float error = 1.0f - sum;
    if (fabs(error) < 0.0001f) break;
    
    int freeCount = 0;
    for (int i = 0; i < N; i++) {
      if (error > 0 && darwin_weights[i] < MAX_W - 0.0001f) freeCount++;
      if (error < 0 && darwin_weights[i] > MIN_W + 0.0001f) freeCount++;
    }
    if (freeCount == 0) break;
    float delta = error / freeCount;
    for (int i = 0; i < N; i++) {
      if (error > 0 && darwin_weights[i] < MAX_W)
        darwin_weights[i] += delta;
      else if (error < 0 && darwin_weights[i] > MIN_W)
        darwin_weights[i] += delta;
      darwin_weights[i] = constrain(darwin_weights[i], MIN_W, MAX_W);
    }
  }
  float sum = 0;
  for (int i = 0; i < 5; i++) sum += darwin_weights[i];
  float corr = 1.0f - sum;
  darwin_weights[0] = constrain(darwin_weights[0] + corr, MIN_W, MAX_W); // ⬅️ corregido
}

void updateDarwinWeights() {
  bool changed = false;
  for (int i = 0; i < 5; i++) {
    if (i == fitnessWinner) winStreak[i]++;
    else winStreak[i] = 0;
    
    if (winStreak[i] > 500) {
      darwin_weights[i] += 0.02f;
      normalizeWeights();
      winStreak[i] = 0;
      changed = true;
    }
    if (winStreak[i] == 0 && fitness[i] < 50) {
      float nw = max(0.05f, darwin_weights[i] - 0.001f);
      if (fabs(nw - darwin_weights[i]) > 0.0001f) {
        darwin_weights[i] = nw;
        normalizeWeights();
        changed = true;
      }
  ...
  if (changed && millis() - lastPrefsSave > 10000) {
    saveDarwinWeights();
...
void computeFitness() {
  ...
  float snr = constrain(irVal / 300.0f, 0.0f, 255.0f);
  float perfusion = (redVal > 0) ? constrain((irVal / (float)redVal) * 50.0f, 0.0f, 255.0f) : 0.0f;
  
  float mean = 0;
  for (int i = 0; i < 5; i++) mean += motion_history[i];
  mean /= 5;
  float var = 0;
  for (int i = 0; i < 5; i++) var += powf(motion_history[i] - mean, 2);
  float stability = constrain(255.0f - var * 5000.0f, 0.0f, 255.0f);
  
  float tempScore = (temp_c > 35 && temp_c < 39) ? 220.0f : 80.0f;
  float predictedMotion = predictMotionPhoenix();
  float motion_penalty = constrain(predictedMotion * 25.0f, 0.0f, 120.0f);
  
  auto weighted = [&](float snr_c, float perf_c) {
    return darwin_weights[0]*snr_c + darwin_weights[1]*(255.0f - motion_penalty*2.0f) +
           darwin_weights[2]*perf_c + darwin_weights[3]*tempScore +
           darwin_weights[4]*stability;
  };
  ...
  fitness[3] = constrain(darwin_weights[0]*220.0f + darwin_weights[1]*240.0f +
                          darwin_weights[2]*200.0f + darwin_weights[3]*tempScore +
                          darwin_weights[4]*stability, 0.0f, 255.0f);
  fitness[4] = constrain(255.0f - predictedMotion*20.0f, 0.0f, 255.0f);
  
  fitnessWinner = 0;
  for (int i = 1; i < 5; i++)
    if (fitness[i] > fitness[fitnessWinner]) fitnessWinner = i;
  
  updateDarwinWeights();
}

float estimateGlucoseIndex(float nir_ratio, float ir_ratio, float skin_temp) {
  nir_ratio = max(nir_ratio, 0.01f); // ⬅️ protección división por cero
  float ratio_diff = nir_ratio / (ir_ratio + 0.001f);
...
void readAllSensorsOptimized() {
  if (max_ok) {
    ...
    if (irValue > 50000) {
      if (checkForBeat(irValue)) {
        ...
        if (delta > 3000) { lastBeat = now; return; } // ⬅️ protección desborde
        beatIntervals[intervalSpot] = delta;
        intervalSpot = (intervalSpot + 1) % 8;
        lastBeat = now;
        beatsPerMinute = 60000.0f / delta;
        
        if (beatsPerMinute < 255 && beatsPerMinute > 20) {
          rates[rateSpot++] = (byte)round(beatsPerMinute);
          rateSpot %= RATE_SIZE;
          beatAvg = 0;
          ...
          float meanInt = 0;
          for (byte x = 0; x < 8; x++) meanInt += beatIntervals[x];
          meanInt /= 8;
          float varInt = 0;
          for (byte x = 0; x < 8; x++)
            varInt += powf(beatIntervals[x] - meanInt, 2);
          varInt /= 8;
          hrv_ms = sqrtf(varInt);
          if (hrv_ms < 5) hrv_ms = beatAvg;
        }
      }
      if (redValue > 0) {
        float ratio = (float)redValue / irValue;
        spo2_pct = constrain(104.0f - 17.0f * ratio, 90.0f, 100.0f);
      }
    }
  }
  ...
  float nir_ratio = 0.5f + (analogRead(PIN_NIR) / 4095.0f) * 1.5f;
  float ir_ratio = max_ok ? (float)particleSensor.getIR() / 100000.0f : 1.0f;
  glucose_index = estimateGlucoseIndex(nir_ratio, ir_ratio, temp_c);
  gsr_ratio = nir_ratio;
  computeFitness();
}
...
void buildPacket(uint8_t* buf) {
  ...
  uint16_t hrv_raw = (uint16_t)(hrv_ms * 100.0f);
  uint16_t tmp_raw = (uint16_t)(temp_c * 100.0f);
  uint16_t gsr_raw = (uint16_t)(gsr_ratio * 1000.0f);
  uint16_t spo_raw = (uint16_t)(spo2_pct * 100.0f);
  uint16_t glc_raw = (uint16_t)glucose_index;
  uint16_t mot_raw = (uint16_t)(motion_magnitude * 1000.0f);
  
  memset(buf, 0, PACKET_SIZE);
  buf[0] = sequenceNumber & 0xFF;
  ...
  buf[16] = fitness[fitnessWinner];
  buf[17] = glc_raw & 0xFF;
  buf[18] = (glc_raw >> 8) & 0xFF;
  buf[19] = mot_raw & 0xFF;
  buf[20] = (mot_raw >> 8) & 0xFF;
  
  for (int i = 0; i < 5; i++) buf[21 + i] = fitness[i];
  buf[26] = fitnessWinner;
  buf[27] = 0;
  
  uint32_t crc = crc32(buf, 28);
  buf[28] = crc & 0xFF;
  buf[29] = (crc >> 8) & 0xFF;
  buf[30] = (crc >> 16) & 0xFF;
  buf[31] = (crc >> 24) & 0xFF;
...
void setup() {
  ...
  lastBeat = millis(); // ⬅️ inicialización protección desborde
  max_ok = initMAX30105();
  Wire.beginTransmission(ADDR_MLX90614);
  mlx_ok = (Wire.endTransmission() == 0);
  Wire.beginTransmission(ADDR_MPU6050);
  Wire.write(0x6B);
  Wire.write(0x00);
  ...
  pEpochChar = pService->createCharacteristic(
    CHARACTERISTIC_EPOCH_UUID,
...
void loop() {
  if (shouldRestartAdv && millis() - lastAdvRestart > 500) {
    BLEDevice::startAdvertising();
    shouldRestartAdv = false;
    lastAdvRestart = millis();
  }
  
  if (millis() - lastSample >= SAMPLE_INTERVAL) { // ⬅️ sin delay()
    lastSample = millis();
    if (!deviceConnected) return;
    
    readAllSensorsOptimized();
    
    bool shouldTX = (fabs(hrv_ms - last_hrv) > 1.0f ||
                     fabs(temp_c - last_temp) > 0.1f ||
                     fabs(glucose_index - last_gluc) > 2.0f ||
                     fabs(motion_magnitude - last_motion) > 0.05f ||
                     millis() - lastBLETX > 1000);
    
    if (shouldTX) {
      buildPacket(packet);
      bool diff = false;
      for (int i = 0; i < 28; i++) {
        if (packet[i] != lastPacket[i]) { diff = true; break; }
      }
      ...
      if (diff || millis() - lastBLETX > 1000) {
        ...
        last_hrv = hrv_ms;
        last_temp = temp_c;
        last_gluc = glucose_index;
        last_motion = motion_magnitude;
        lastBLETX = millis();
        sequenceNumber++;
      }
    }
