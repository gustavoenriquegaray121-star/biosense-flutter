// BioSense Band - Firmware v5.6 - Definitivo Final
#include <BLEDevice.h>
#include <BLEServer.h>
#include <BLEUtils.h>
#include <BLE2902.h>
...
float readMLX90614(){
  ...
  if(tempC>=30&&tempC<=45) return tempC;
  return temp_c;
}
...
void loadDarwinWeights(){
  ...
  if(prefs.isKey("w0")){
    darwin_weights[0]=prefs.getFloat("w0",0.30);
    darwin_weights[1]=prefs.getFloat("w1",0.25);
    darwin_weights[2]=prefs.getFloat("w2",0.20);
    darwin_weights[3]=prefs.getFloat("w3",0.15);
    darwin_weights[4]=prefs.getFloat("w4",0.10);
...
void normalizeWeights(){
  ...
  for(int iter=0; iter<10; iter++){
    float sum=0; for(int i=0;i<N;i++) sum+=darwin_weights[i];
  ...
  darwin_weights[0]+=1.0f-sum;
  darwin_weights[0]=constrain(darwin_weights[0],MIN_W,MAX_W);
}
...
void updateDarwinWeights(){
  ...
  for(int i=0;i<5;i++){
    ...
    if(winStreak[i]==0&&fitness[i]<50){
      ...
      if(fabs(nw-darwin_weights[i])>0.0001) changed=true;
      darwin_weights[i]=nw;
      normalizeWeights();
    }
  }
...
void computeFitness(){
  ...
  auto weighted=[&](float snr_c,float perf_c){
    return darwin_weights[0]*snr_c+darwin_weights[1]*(255-motion_penalty*2)+darwin_weights[2]*perf_c+darwin_weights[3]*tempScore+darwin_weights[4]*stability;
  };
  fitness[0]=constrain(weighted(snr,perfusion),0,255);
  fitness[1]=constrain(weighted(snr*0.8,perfusion*0.9),0,255);
...
float estimateGlucoseIndex(float nir_ratio,float ir_ratio,float skin_temp){
  float ratio_diff=nir_ratio/(ir_ratio+0.001);
  float temp_factor=1.0+(skin_temp-36.6)*0.02;
  float g=calib_b+(ratio_diff-1.0)*calib_m*temp_factor;
  return constrain(g,40,400);
}
...
void readAllSensorsOptimized(){
  ...
  float nir_ratio=0.5+(analogRead(PIN_NIR)/4095.0)*1.5;
  float ir_ratio=max_ok?(float)particleSensor.getIR()/100000.0:1.0;
...
void buildPacket(uint8_t* buf){
  ...
  buf[8]=hrv_raw&0xFF; buf[9]=(hrv_raw>>8)&0xFF;
  buf[10]=tmp_raw&0xFF; buf[11]=(tmp_raw>>8)&0xFF;
  buf[12]=gsr_raw&0xFF; buf[13]=(gsr_raw>>8)&0xFF;
  buf[14]=spo_raw&0xFF; buf[15]=(spo_raw>>8)&0xFF;
  buf[16]=fitness[fitnessWinner];
  buf[17]=glc_raw&0xFF; buf[18]=(glc_raw>>8)&0xFF;
  buf[19]=mot_raw&0xFF; buf[20]=(mot_raw>>8)&0xFF;
  buf[21]=fitness[0]; buf[22]=fitness[1]; buf[23]=fitness[2]; buf[24]=fitness[3]; buf[25]=fitness[4];
  buf[26]=fitnessWinner;
  buf[27]=0;
  uint32_t crc=crc32(buf,28);
  buf[28]=crc&0xFF; buf[29]=(crc>>8)&0xFF; buf[30]=(crc>>16)&0xFF; buf[31]=(crc>>24)&0xFF;
...
void setup(){
  ...
  Wire.beginTransmission(ADDR_MPU6050); Wire.write(0x6B); Wire.write(0x00); mpu_ok=(Wire.endTransmission()==0);
  ...
  if(deviceConnected){
    ...
    if(shouldTX){
      ...
      if(diff||millis()-lastBLETX>1000){
        pCharacteristic->setValue(packet,44);
        pCharacteristic->notify();
        memcpy(lastPacket,packet,44);
        last_hrv=hrv_ms; last_temp=temp_c; last_gluc=glucose_index; last_motion=motion_magnitude;
