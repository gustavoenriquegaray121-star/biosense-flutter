// BioSense Band - Firmware v5.8.5 - Production Telemetry FIXED
// Fixes: FIFO MAX30102, MTU fragmentation, SensorFlags, Darwin OFF
...
#define FRAG_SIZE 17 // 20 - 3 header
...
class MyServerCallbacks : public BLEServerCallbacks {
  void onConnect(BLEServer* p) override {
    ...
    negotiatedMTU = p->getPeerMTU(p->getConnId());
...
float predictMotionPhoenix() {
  ...
  float t1 = motion_history[idx1];
  float t2 = motion_history[idx2];
  float t3 = motion_history[idx3];
  float t4 = motion_history[idx4];
  float x = t1;
  float v = t1 - t2;
  float a = t1 - 2*t2 + t3;
  float j = t1 - 3*t2 + 3*t3 - t4;
  float dt = SAMPLE_INTERVAL / 1000.0f; // FIX: 0.1f not 0.08f
...
void computeFitness() {
  ...
  if (validMotionSamples > 0) {
    for (int i = 0; i < validMotionSamples; i++) mean += motion_history[i];
    mean /= validMotionSamples;
  }
  float var = 0;
  if (validMotionSamples > 1) {
    for (int i = 0; i < validMotionSamples; i++) var += powf(motion_history[i] - mean, 2);
    var /= validMotionSamples;
  }
  ...
  auto weighted = [&](float snr_c, float perf_c) {
    return darwin_weights[0]*snr_c + darwin_weights[1]*(255.0f - motion_penalty*2.0f) + darwin_weights[2]*perf_c + darwin_weights[3]*tempScore + darwin_weights[4]*stability;
  };
  ...
  fitnessWinner = 0;
  for (int i = 1; i < 5; i++) if (fitness[i] > fitness[fitnessWinner]) fitnessWinner = i;
  updateDarwinWeights();
}
...
void readAllSensorsOptimized() {
  sensorFlags = 0;
  sensorFlags |= FLAG_GLUCOSE_EXPERIMENTAL;
  long latestIr = 0;
  long latestRed = 0;
  ...
  if (max_ok) {
      ...
      if (latestIr > 50000) {
        if (checkForBeat(latestIr)) {
          ...
          if (delta > 300 && delta < 3000) {
            if (validIntervals > 0) {
              float meanInt = 0;
              for (byte x = 0; x < validIntervals; x++) meanInt += beatIntervals[x];
              meanInt /= validIntervals;
              if (fabsf(delta - meanInt) <= meanInt * 0.20f) {
                beatIntervals[intervalSpot] = delta;
                intervalSpot = (intervalSpot + 1) % 10;
                if (validIntervals < 10) validIntervals++;
              }
            } else {
              beatIntervals[intervalSpot] = delta;
              intervalSpot = (intervalSpot + 1) % 10;
            ...
            if (beatsPerMinute < 255 && beatsPerMinute > 20) {
              ...
              if (validIntervals >= 8) {
                float mInt = 0;
                for (byte x = 0; x < validIntervals; x++) mInt += beatIntervals[x];
                mInt /= validIntervals;
                float varInt = 0;
                for (byte x = 0; x < validIntervals; x++) varInt += powf(beatIntervals[x] - mInt, 2);
                varInt /= validIntervals;
                float computedHRV = sqrtf(varInt);
                if (computedHRV >= 5.0f && computedHRV < 300.0f) {
                  lastValidHRV = computedHRV;
                  hrv_estimated_ms = computedHRV;
                }
              }
            }
          }
    ...
    if (hasNewSample && latestIr > 1000) {
      ...
      red_dc = red_dc * 0.95f + latestRed * 0.05f;
      ir_dc = ir_dc * 0.95f + latestIr * 0.05f;
  ...
  if (mpu_ok) {
    readMPU6050();
    if (validMotionSamples >= 3) sensorFlags |= FLAG_MPU_VALID | FLAG_MOTION_VALID;
  }

  if (mlx_ok && millis() - lastMLXRead > 2000) {
    float newTemp = readMLX90614();
    if (!isnan(newTemp)) {
      if (temp_c == 0) temp_c = newTemp;
      else temp_c = temp_c * 0.7f + newTemp * 0.3f;
      sensorFlags |= FLAG_MLX_VALID;
    }
    lastMLXRead = millis();
  }
  ...
  if (DARWIN_ENABLED || validMotionSamples >= 3) computeFitness();
}
...
void buildPacket(uint8_t* buf) {
  ...
  buf[1] = sensorFlags;
  buf[2] = sequenceNumber & 0xFF;
  buf[3] = (sequenceNumber >> 8) & 0xFF;
  buf[4] = (sequenceNumber >> 16) & 0xFF;
  buf[5] = (sequenceNumber >> 24) & 0xFF;
  buf[6] = now & 0xFF;
  buf[7] = (now >> 8) & 0xFF;
  buf[8] = (now >> 16) & 0xFF;
  buf[9] = (now >> 24) & 0xFF;
  ...
  buf[28] = fitnessWinner;
  buf[29] = bat_raw & 0xFF; buf[30] = (bat_raw >> 8) & 0xFF;
  buf[31] = beatAvg & 0xFF;
  buf[32] = (uint8_t)beatsPerMinute;
  // 33-39 Reserved
